...
data ℂ (α : Set) : Wires → Wires → Set where
    -- Computation-related
    Not  : ℂ α [W] [W]
    And  : ℂ α ([W] [+] [W]) [W]  -- TODO: Should these remain with fixed size (binary)?
    Or   : ℂ α ([W] [+] [W]) [W]
    -- Plug
...
Fork2 = Plug fork2
    where fork2 : Wires → Wires
          fork2 [W]         = [W]  -- FIXME: This case should never happen
          fork2 (w [*] n)   = [W]  -- FIXME: This case should never happen
          fork2 (w₁ [+] w₂) = w₁
...
exampleAnd : ℂ Bool ([W] [+] [W]) [W]
exampleAnd = And

exampleNand : ℂ Bool ([W] [+] [W]) [W]
exampleNand = And ⟫ Not

exampleXorLikeStruct : ℂ Bool (([W] [+] [W]) [+] ([W] [+] [W])) [W]
exampleXorLikeStruct =
       And
    || And         ⟫ Or

exampleXor : ℂ Bool ([W] [+] [W]) [W]
exampleXor = 
    Fork2  ⟫    (Not || pID  ⟫ And)
             || (pID || Not  ⟫ And)  ⟫ Or
...
evalBool (Plug p)   w = {!!}
evalBool (_||_ {i₁} c₁ c₂) w with splitAt (evalWires i₁) w
