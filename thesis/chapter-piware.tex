...
\label{chap:piware}
    ...
    \label{sec:circuit-syntax}
        ...
        \subsection{Design discipline enforced by circuit constructors}
            ...
            \begin{listing}[h]
                \centering{\ExecuteMetaData[code/agda/latex/PiWare/Circuit/Core.tex]{comb-core}}
    ...
    \label{sec:circuit-semantics}
        ...
        \label{subsec:sequential-eval}
            ...
            helper definition, which gives \emph{one step} of a causal stream function, i.e,
            ...
            Agda's \emph{non-empty lists} are used to implement the context type.
            In this way, there is always a current value (head of a list), while the past (tail)
            might be empty.
            In the \AM{Data.List.NonEmpty} module, the type \AD{List⁺} \AB{α} is defined
            as just a \emph{type synonym} of \AB{α} \AD{×} \AD{List} \AB{α}, therefore, in some
            places, to avoid confusion with normal lists, we use the \emph{pair constructor} (comma)
            to pattern match on non-empty lists.
            ...
            In the \AI{Nil}, \AI{Gate} and \AI{Plug} cases, the purely combinational semantics (\AF{⟦\_⟧'})
            can be used (ignoring past inputs), and is passed (implicitly) the necessary proof of purity.
            For example, in the \AI{Nil} case, the implicit argument has type
            \AF{comb'} \AI{Nil} \AY{=} \AD{⊤}, therefore its only value (which Agda guesses) is \AF{tt}.

            The \AI{DelayLoop} case makes use of a helper function (\AF{delay}) expressing the behaviour
            of the memory element (flip-flop) \emph{inside} the \AI{DelayLoop} block
            (as depicted in Figure~\ref{fig:semantics-syn-fundamental}).
            Listing~\ref{lst:delay} shows the definition of \AF{delay}.

            \begin{listing}[h]
                \raggedleft{\ExecuteMetaData[code/agda/latex/PiWare/Simulation/Core.patched.tex]{delay}}
            ...
            performs the work.
            This indirect definition is necessary in order to pass Agda's termination checker,
            which needs that recursive calls be performed on at least one \emph{structurally smaller}
            argument – in this case, the list of past inputs
            (\AB{w⁻¹} \AI{∷} \AB{w⁻} becomes \AB{w⁻¹}).

            The \AB{c} parameter to \AF{delay} is the \emph{combinational} part inside the \AI{DelayLoop}
            block, thus its type (\AB{c} \AY{:} \AY{(}\AB{i} \AF{+} \AB{l}\AY{)} \AY{(}\AB{o} \AF{+} \AB{l}\AY{)})
            increases the size of the input/output by the amount of wires that "loop back" through the flip-flop.
            In the base case (no past), \AF{delay'} just simulates \AF{⟦} \AB{c} \AF{⟧'} with an \emph{undefined}
            state – the \AD{Atom} indexed by zero (\AI{Fz}).
            In the recursive case, the state is calculate by a recursive call to \AF{delay'}
            where the "current" input has been substituted by the most recent past (\AB{w⁻¹}) and the "past"
            by the even farther past (\AB{w⁻}).

            %% TODO: structural constructor cases
            %% runC and the "stream interface"
            %% high-level sequential simulation
