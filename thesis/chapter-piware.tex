...
\label{chap:piware}
    ...
    \label{sec:circuit-abstraction}
        ...
        \label{subsec:synthesizable}
            ...
            With a given encoding for set \AB{α} (\AB{sα} \AY{:} \AD{𝕎} \AB{i})
            and one for β (\AB{sβ} \AY{:} \AD{𝕎} \AB{j}), it is clear that at least \AB{i} \AF{⊔} \AB{j} atoms
            are necessary to encode \AB{α} \AD{⊎} \AB{β}; the size of a sum is at least the maximum
            ...
            The atom index \AB{n} gives the \AD{Atom} to be used in case the sum is built from the left (\AL{inj₁}).
